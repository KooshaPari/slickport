...
export async function GET({ request }: RequestEvent) {
    ...
    return addCorsHeaders(json((await pulltFirsDatabase()) as unknown as Project), origin);
}

async function pullAllDatabase() {
    // postgres drizzle orm
    const projects: Project[] = await db.select().from(table).limit(1).execute() as unknown as Project[];
    return projects;
}
